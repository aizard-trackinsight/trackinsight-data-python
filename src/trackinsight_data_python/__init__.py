"""Public package API for trackinsight-data-python."""

from importlib import import_module

_API_EXPORTS = [
    "getMetadata",
    "getShares",
    "getStockFlows",
    "getExposures",
    "getTimeseries",
    "getMonthlyTimeseries",
    "getReports",
    "getHoldings",
    "getLiquidity",
    "getLiquiditySummary",
    "downloadShares",
    "downloadStockFlows",
    "downloadExposures",
    "downloadReports",
    "downloadTimeseries",
    "downloadMonthlyTimeseries",
    "downloadHoldings",
    "downloadLiquidity",
    "downloadLiquiditySummary",
    "contains_any",
    "contains_all",
    "single_among",
    "contains_exact",
    "contains_any_joined",
]

__all__ = ["main", *_API_EXPORTS]

_EXPORT_MODULES = {
    "getMetadata": ".api",
    "getShares": ".api",
    "getStockFlows": ".api",
    "getExposures": ".api",
    "getTimeseries": ".api",
    "getMonthlyTimeseries": ".api",
    "getReports": ".api",
    "getHoldings": ".api",
    "getLiquidity": ".api",
    "getLiquiditySummary": ".api",
    "downloadShares": ".download",
    "downloadStockFlows": ".download",
    "downloadExposures": ".download",
    "downloadReports": ".download",
    "downloadTimeseries": ".download",
    "downloadMonthlyTimeseries": ".download",
    "downloadHoldings": ".download",
    "downloadLiquidity": ".download",
    "downloadLiquiditySummary": ".download",
    "contains_any": ".helpers",
    "contains_all": ".helpers",
    "single_among": ".helpers",
    "contains_exact": ".helpers",
    "contains_any_joined": ".helpers",
}


def __getattr__(name: str):
    module_name = _EXPORT_MODULES.get(name)
    if module_name is not None:
        return getattr(import_module(module_name, __name__), name)
    raise AttributeError(f"module {__name__!r} has no attribute {name!r}")


def main() -> None:
    print("Hello from trackinsight-data-python!")
